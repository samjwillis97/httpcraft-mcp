    ...
    child.on('close', (exitCode, signal) => {
      ...
      if (exitCode === 0) {
        ...
        resolve({
          success: false,
          error: new Error(
            `Process exited with code ${exitCode}${signal ? ` (signal: ${signal})` : ''}: ${stderr}`
          ),
        });
      }
    });
...
export async function checkExecutableExists(path: string): Promise<boolean> {
  try {
...
export async function findExecutableInPath(name: string): Promise<string | null> {
  try {
