...
export class ResponseParser {
   ...
   */
  private parseTextOutput(stdout: string): HttpCraftResponse {
   ...
   */
  private extractStatusCode(output: string): number | undefined {
    const patterns = [
      /Status:\s*(\d+)/i,
      /HTTP\/\d+\.\d+\s+(\d+)/,
      /Response\s+(\d+)/i,
   ...
   */
  private extractDataFromText(output: string, lines: string[]): any {
   ...
   */
  public parseError(stderr: string): string {
    const errorPatterns = [
      /Error:\s*(.+)/,
      /error:\s*(.+)/i,
      /failed:\s*(.+)/i,
   ...
   */
  public validateResponse(response: HttpCraftResponse): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
